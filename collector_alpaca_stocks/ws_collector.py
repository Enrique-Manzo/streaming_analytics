...
aapl_trades_topic_path = publisher.topic_path(GCP_PROJECT_ID, AAPL_TRADES_TOPIC_ID)
...
async def _process_trade(raw: dict, obs: ObservabilityState) -> None:
    ...
    def make_publisher(topic, data):
        def publish_message():
            future = publisher.publish(topic, data)
            return future.result()  # force errors to surface

        return publish_message

    # Separate pub/sub topic for AAPL as it follows a different stream with vol prediction
    topic = aapl_trades_topic_path if trade.S == "AAPL" else trades_topic_path

    await asyncio.get_running_loop().run_in_executor(executor, make_publisher(topic, message_bytes))
...
async def _process_quote(raw: dict, obs: ObservabilityState) -> None:
    ...
    # Quote pub/sub topic for AAPL not needed as predictions are made from trades only
